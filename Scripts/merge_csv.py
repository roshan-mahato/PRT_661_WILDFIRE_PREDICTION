...
def main():
    ...
    print(
        f"Loaded {len(archive_df)} archive rows "
    ...
    if len(combined_df) < before:
        print(
            f"Dropped {before - len(combined_df)} duplicate rows found in the overlap window."
        )

    combined_df = combined_df.sort_values(["acq_date", "acq_time"]).reset_index(
        drop=True
    )
    ...
    print(
        f"Date range: {combined_df['acq_date'].min()} to {combined_df['acq_date'].max()}"
    )
    ...
    print(
        "IMPORTANT: update preprocess_data.py's vegetation-fire filter to treat "
