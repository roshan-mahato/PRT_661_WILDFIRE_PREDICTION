...
def get_locations_and_dates(df):
    ...
    return (
        df.groupby(["lat_round", "lon_round"])["acq_date_dt"].apply(list).reset_index()
    )
...
def main():
    ...
    for lat, lon, start_date, end_date in remaining_calls:
        if calls_this_run >= DAILY_CALL_LIMIT:
            ...
            print(
                f"Run this script again tomorrow — it will resume automatically "
        ...
        if calls_this_run % 20 == 0:
            print(
                f"Progress: {done_total}/{total_calls_needed} calls done "
    ...
    if done_total >= total_calls_needed:
        print(
            f"Fetching complete! All weather data has been saved to {OUTPUT_CSV_PATH}"
        )
    else:
