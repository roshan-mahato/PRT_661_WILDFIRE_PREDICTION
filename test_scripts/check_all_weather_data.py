...
def check_call_level_completeness(firms_df):
    ...
    locations = (
        firms_df.groupby(["lat_round", "lon_round"])["acq_date_dt"]
        .apply(list)
        .reset_index()
    )
    ...
    if remaining_calls:
        ...
        print(
            f">>> Fetch is {pct:.1f}% complete. Run fetch_weather_csv.py again to continue."
        )

        remaining_df = pd.DataFrame(
            remaining_calls,
            columns=["lat_round", "lon_round", "start_date", "end_date"],
        )
        remaining_df.to_csv(REMAINING_CALLS_CSV_PATH, index=False)
        print(
            f">>> Saved list of {len(remaining_calls)} remaining/failed calls to "
...
def check_row_level_completeness(firms_df, weather_df):
    ...
    weather_keys = set(
        zip(
            weather_df["lat_round"],
            weather_df["lon_round"],
            weather_df["w_date"],
            weather_df["w_hour"],
        )
    )
    ...
    firms_df["acq_hour"] = (
        firms_df["acq_time"].astype(str).str.zfill(4).str[:2].astype(int)
    )
    firms_keys = list(
        zip(
            firms_df["lat_round"],
    ...
    if missing > 0:
        ...
        missing_df = pd.DataFrame(
            missing_keys, columns=["lat_round", "lon_round", "acq_date", "acq_hour"]
        )
...
def main():
    ...
    firms_df["lat_round"] = firms_df["latitude"].apply(
        lambda v: round_to_grid(v, GRID_SIZE_DEGREES)
    )
    firms_df["lon_round"] = firms_df["longitude"].apply(
        lambda v: round_to_grid(v, GRID_SIZE_DEGREES)
    )
    ...
    if call_level_complete and row_level_complete:
        print(
            "READY: All weather data is fetched and matches your FIRMS data. "
