...
def try_fetch_with_split_fallback(lat, lon, start_date, end_date, depth=0):
    ...
    try:
        ...
        print(
            f"{'  ' * depth}OK: ({lat}, {lon}, {start_date} to {end_date}) "
            f"-> {len(chunk_df)} rows"
        )
        return []
        ...
        if total_days <= MIN_SPLIT_DAYS:
            print(
                f"{'  ' * depth}Range too small to split further ({total_days} day(s)). Giving up on this one."
            )
        ...
        print(
            f"{'  ' * depth}Splitting into {first_half} and {second_half}, retrying each..."
        )

        still_failed = []
        still_failed += try_fetch_with_split_fallback(
            lat, lon, first_half[0], first_half[1], depth + 1
        )
        time.sleep(REQUEST_DELAY_SECONDS)
        still_failed += try_fetch_with_split_fallback(
            lat, lon, second_half[0], second_half[1], depth + 1
        )
        return still_failed
...
def main():
    ...
    if still_failed:
        ...
        print(
            f"{len(still_failed)} sub-range(s) still failed — saved to {REMAINING_CALLS_CSV_PATH} for another retry."
        )
    else:
        print(
            "All listed calls succeeded. Run check_weather_completeness.py "
